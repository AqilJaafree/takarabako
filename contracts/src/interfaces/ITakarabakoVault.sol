...
/// @notice PRD §7.4 — ERC-4626-flavored vault over JPYC. Not a strict IERC4626:
...
interface ITakarabakoVault {
    /// @notice Treasury fronts `jpycAmount` from cash inserted into the box;
    /// shares are minted to `user`.
    function depositFor(address user, uint256 jpycAmount) external returns (uint256 shares);

    /// @notice Burns `shares` belonging to `owner` and sends principal + accrued
    /// yield in JPYC to `recipient` (the dev/treasury wallet on withdraw).
    ...
    /// @notice Current JPYC value of `user`'s shares, principal + yield included.
